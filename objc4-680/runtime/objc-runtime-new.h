...
void attachLists(List* const * addedLists, uint32_t addedCount) {
    ...
    if (hasArray()) {
        ...
        //内存移动
        //把本类的原数组从0移动到addedcount+1的位置，前面空出addedcount个位置用来插入添加的数组
        //[分类A元素1，分类A元素2,分类B元素1,...,[宿主元素1],[宿主元素2]]
...
struct category_t {
    const char *name;                               //分类名
    classref_t cls;                                 //所属类对象，编译时无值，在runtime加载的时候指向对应的类
