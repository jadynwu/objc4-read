...
attachCategories(Class cls, category_list *cats, bool flush_caches)
{
    ...
    //判断是否是元类对象，取决于添加的是实例方法还是类方法
    bool isMeta = cls->isMetaClass();

    //创建二维数组
    //以方法为例[[method_t,method_t],[method_t,method_t,method_t],[method_t]...]
    //一个子数组代表一个分类的方法列表，而分类的顺序是根据编译顺序排列的
    //方法二维数组
    ...
    int i = cats->count;
    bool fromBundle = NO;
    //从i可以看出，拼接是倒序遍历的，所以最先被访问到的是最后编译的分类
    while (i--) {
        //获取一个分类
        auto& entry = cats->list[i];

        //通过meta判断获取的是实例方法还是类方法
    ...
    //获取宿主类的读写信息，包含宿主类的方法、属性、协议
    auto rw = cls->data();
    
    //特殊处理分类当中实现的一些内存管理相关方法
...
static void remethodizeClass(Class cls)
{
    ...
    runtimeLock.assertWriting();
    //判断是否是元类对象，取决于方法为类方法还是实力方法
    isMeta = cls->isMetaClass();

    //unattachedCategoriesForClass方法为获取cls类的未拼接的分类
    //如果存在未拼接的分类，则进入if
